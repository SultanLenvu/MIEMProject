#include "BHGameHUD.h"
#include "BHGame.h"
//#include "SBHGameScoreboardWidget.h"
...
#define LOCTEXT_NAMESPACE "BHGameGame.HUD.Menu"

const float ABHGameHUD::MinHudScale = 0.5f;

ABHGameHUD::ABHGameHUD(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
	...
	//OnPlayerTalkingStateChangedDelegate = FOnPlayerTalkingStateChangedDelegate::CreateUObject(this, &ABHGameHUD::OnPlayerTalkingStateChanged);
	...
	HitNotifyIcon[EBHGameHudPosition::Left] = UCanvas::MakeIcon(HitNotifyTexture, 158, 831, 585, 392);
	HitNotifyIcon[EBHGameHudPosition::FrontLeft] = UCanvas::MakeIcon(HitNotifyTexture, 369, 434, 460, 378);
	HitNotifyIcon[EBHGameHudPosition::Front] = UCanvas::MakeIcon(HitNotifyTexture, 848, 284, 361, 395);
	HitNotifyIcon[EBHGameHudPosition::FrontRight] = UCanvas::MakeIcon(HitNotifyTexture, 1212, 397, 427, 394);
	HitNotifyIcon[EBHGameHudPosition::Right] = UCanvas::MakeIcon(HitNotifyTexture, 1350, 844, 547, 321);
	HitNotifyIcon[EBHGameHudPosition::BackRight] = UCanvas::MakeIcon(HitNotifyTexture, 1232, 1241, 458, 341);
	HitNotifyIcon[EBHGameHudPosition::Back] = UCanvas::MakeIcon(HitNotifyTexture, 862, 1384, 353, 408);
	HitNotifyIcon[EBHGameHudPosition::BackLeft] = UCanvas::MakeIcon(HitNotifyTexture, 454, 1251, 371, 410);
	...
	Crosshair[EBHGameCrosshairDirection::Left] = UCanvas::MakeIcon(HUDMainTexture, 43, 402, 25, 9); // left
	Crosshair[EBHGameCrosshairDirection::Right] = UCanvas::MakeIcon(HUDMainTexture, 88, 402, 25, 9); // right
	Crosshair[EBHGameCrosshairDirection::Top] = UCanvas::MakeIcon(HUDMainTexture, 74, 371, 9, 25); // top
	Crosshair[EBHGameCrosshairDirection::Bottom] = UCanvas::MakeIcon(HUDMainTexture, 74, 415, 9, 25); // bottom
	Crosshair[EBHGameCrosshairDirection::Center] = UCanvas::MakeIcon(HUDMainTexture, 75, 403, 7, 7); // center

	Offsets[EBHGameHudPosition::Left] = FVector2D(173, 0);
	Offsets[EBHGameHudPosition::FrontLeft] = FVector2D(120, 125);
	Offsets[EBHGameHudPosition::Front] = FVector2D(0, 173);
	Offsets[EBHGameHudPosition::FrontRight] = FVector2D(-120, 125);
	Offsets[EBHGameHudPosition::Right] = FVector2D(-173, 0);
	Offsets[EBHGameHudPosition::BackRight] = FVector2D(-120, -125);
	Offsets[EBHGameHudPosition::Back] = FVector2D(0, -173);
	Offsets[EBHGameHudPosition::BackLeft] = FVector2D(120, -125);
...
//void ABHGameHUD::EndPlay(const EEndPlayReason::Type EndPlayReason)
//{
//	ConditionalCloseScoreboard(true);
//
//	ABHGamePlayerController* BHGamePC = Cast<ABHGamePlayerController>(PlayerOwner);
//	if (BHGamePC != NULL)
//	{
//		// Reset the ignore input flags, so we can control the camera during warmup
//		BHGamePC->SetCinematicMode(false, false, false, true, true);
//	}
//
//	Super::EndPlay(EndPlayReason);
//}

//void ABHGameHUD::SetMatchState(EBHGameMatchState::Type NewState)
//{
//	MatchState = NewState;
//}

//EBHGameMatchState::Type ABHGameHUD::GetMatchState() const
//{
//	return MatchState;
//}

//FString ABHGameHUD::GetTimeString(float TimeSeconds)
//{
...
void ABHGameHUD::DrawWeaponHUD()
{
	ABHGameCharacter* MyPawn = CastChecked<ABHGameCharacter>(GetOwningPawn());
	ABHGameWeapon* MyWeapon = MyPawn->GetWeapon();
	if (MyWeapon)
	{
		...
		//SECONDARY WEAPON
		ABHGameWeapon* SecondaryWeapon = NULL;
...
void ABHGameHUD::DrawHealth()
{
	ABHGameCharacter* MyPawn = Cast<ABHGameCharacter>(GetOwningPawn());
	Canvas->SetDrawColor(FColor::White);
...
//void ABHGameHUD::DrawMatchTimerAndPosition()
//{
//	ABHGameGameState* const MyGameState = GetWorld()->GetGameState<ABHGameGameState>();
//	Canvas->SetDrawColor(FColor::White);
...
//	if (MyGameState && MatchState == EBHGameMatchState::Playing)
//	{
...
//		Text = FString();
//		ABHGamePlayerController* MyPC = Cast<ABHGamePlayerController>(PlayerOwner);
//		if (MyPC && MyGameState && MatchState == EBHGameMatchState::Playing)
//		{
//			ABHGamePlayerState* MyPlayerState = Cast<ABHGamePlayerState>(MyPC->PlayerState);
//			if (MyPlayerState)
//			{
...
//void ABHGameHUD::NotifyOutOfAmmo()
//{
//	NoAmmoNotifyTime = GetWorld()->GetTimeSeconds();
//}

//void ABHGameHUD::NotifyEnemyHit()
//{
...
void ABHGameHUD::DrawHUD()
{
	...
	//if (MatchState == EBHGameMatchState::Playing)
	//{
		ABHGamePlayerController* MyPC = Cast<ABHGamePlayerController>(PlayerOwner);
		if (MyPC)
		{
...
void ABHGameHUD::DrawDebugInfoString(const FString& Text, float PosX, float PosY, bool bAlignLeft, bool bAlignTop, const FColor& TextColor)
{
...
void ABHGameHUD::DrawCrosshair()
{
	ABHGamePlayerController* PCOwner = Cast<ABHGamePlayerController>(PlayerOwner);
	if (PCOwner)
	{
		ABHGameCharacter* Pawn = Cast<ABHGameCharacter>(PCOwner->GetPawn());
			...
			&& (Pawn->IsTargeting() || (!Pawn->IsTargeting() && !Pawn->GetWeapon()->bHideCrosshairWhileNotAiming)))
		{
			...
			ABHGameWeapon_Instant* InstantWeapon = Cast<ABHGameWeapon_Instant>(Pawn->GetWeapon());
			ABHGameWeapon* MyWeapon = Pawn->GetWeapon();
			const float CurrentTime = GetWorld()->GetTimeSeconds();
			...
			if (Pawn->IsTargeting() && MyWeapon && MyWeapon->UseLaserDot)
			{
				...
				Canvas->DrawIcon(*CurrentCrosshair[EBHGameCrosshairDirection::Center],
					CenterX - (*CurrentCrosshair[EBHGameCrosshairDirection::Center]).UL * ScaleUI / 2.0f,
					CenterY - (*CurrentCrosshair[EBHGameCrosshairDirection::Center]).VL * ScaleUI / 2.0f, ScaleUI);
			}
			else
			{
				Canvas->DrawIcon(*CurrentCrosshair[EBHGameCrosshairDirection::Center],
				...
				Canvas->DrawIcon(*CurrentCrosshair[EBHGameCrosshairDirection::Left],
					CenterX - 1 - (*CurrentCrosshair[EBHGameCrosshairDirection::Left]).UL * ScaleUI - CrossSpread * ScaleUI,
					CenterY - (*CurrentCrosshair[EBHGameCrosshairDirection::Left]).VL * ScaleUI / 2.0f, ScaleUI);
				Canvas->DrawIcon(*CurrentCrosshair[EBHGameCrosshairDirection::Right],
					CenterX + CrossSpread * ScaleUI,
					CenterY - (*CurrentCrosshair[EBHGameCrosshairDirection::Right]).VL * ScaleUI / 2.0f, ScaleUI);

				Canvas->DrawIcon(*CurrentCrosshair[EBHGameCrosshairDirection::Top],
					CenterX - (*CurrentCrosshair[EBHGameCrosshairDirection::Top]).UL * ScaleUI / 2.0f,
					CenterY - 1 - (*CurrentCrosshair[EBHGameCrosshairDirection::Top]).VL * ScaleUI - CrossSpread * ScaleUI, ScaleUI);
				Canvas->DrawIcon(*CurrentCrosshair[EBHGameCrosshairDirection::Bottom],
					CenterX - (*CurrentCrosshair[EBHGameCrosshairDirection::Bottom]).UL * ScaleUI / 2.0f,
...
void ABHGameHUD::DrawDeathMessages()
{
	if (PlayerOwner == NULL)
	{
		return;
...
void ABHGameHUD::ShowDeathMessage(class ABHGamePlayerState* KillerPlayerState, class ABHGamePlayerState* VictimPlayerState, const UDamageType* KillerDamageType)
{
	...
	if (GetWorld()->GetGameState())
	{
		...
		ABHGamePlayerState* MyPlayerState = PlayerOwner ? Cast<ABHGamePlayerState>(PlayerOwner->PlayerState) : NULL;
		...
		if (DefGame && KillerPlayerState && VictimPlayerState && MyPlayerState)
		{
			...
			//NewMessage.DamageType = MakeWeakObjectPtr(const_cast<UBHGameDamageType*>(Cast<const UBHGameDamageType>(KillerDamageType)));
...
//void ABHGameHUD::NotifyWeaponHit(float DamageTaken, struct FDamageEvent const& DamageEvent, class APawn* PawnInstigator)
//{
//	const float CurrentTime = GetWorld()->GetTimeSeconds();
//	ABHGameCharacter* MyPawn = (PlayerOwner) ? Cast<ABHGameCharacter>(PlayerOwner->GetPawn()) : NULL;
//	if (MyPawn)
//	{
...
//void ABHGameHUD::DrawHitIndicator()
//{
//	const float CurrentTime = GetWorld()->GetTimeSeconds();
//	if (CurrentTime - LastHitTime <= HitNotifyDisplayTime)
//	{
...
//void ABHGameHUD::OnPlayerTalkingStateChanged(TSharedRef<const FUniqueNetId> TalkingPlayerId, bool bIsTalking)
//{
...
void ABHGameHUD::PostInitializeComponents()
{
...
//void ABHGameHUD::ConditionalCloseScoreboard(bool bFocus)
//{
//	if (bIsScoreBoardVisible)
//	{
...
//void ABHGameHUD::ToggleScoreboard()
//{
//	ShowScoreboard(!bIsScoreBoardVisible);
//}

//bool ABHGameHUD::ShowScoreboard(bool bEnable, bool bFocus)
//{
...
//	{
//		ABHGamePlayerController* BHGamePC = Cast<ABHGamePlayerController>(PlayerOwner);
//		if (BHGamePC == NULL || BHGamePC->IsGameMenuVisible())
//		{
//			return false;
//		}
//	}
...
//			.Padding(FMargin(50))
//			[
//				SAssignNew(ScoreboardWidget, SBHGameScoreboardWidget)
...
//void ABHGameHUD::ToggleChat()
//{
//	ABHGamePlayerController* BHGamePC = Cast<ABHGamePlayerController>(PlayerOwner);
//	// If the game menu is visible dont show the chat menu
//	if ((BHGamePC == NULL) || BHGamePC->IsGameMenuVisible() || GetMatchState() == EBHGameMatchState::Warmup)
//	{
//		return;
//	}
//
...
//void ABHGameHUD::SetChatVisibilty(const EVisibility RequiredVisibility)
//{
//	TryCreateChatWidget();
//
...
//void ABHGameHUD::AddChatLine(const FText& ChatString, bool bWantFocus)
//{
//	TryCreateChatWidget();
...
void ABHGameHUD::MakeUV(FCanvasIcon& Icon, FVector2D& UV0, FVector2D& UV1, uint16 U, uint16 V, uint16 UL, uint16 VL)
{
	if (Icon.Texture)
	{
...
//bool ABHGameHUD::TryCreateChatWidget()
//{
//	bool bCreated = false;
//	ABHGamePlayerController* BHGamePC = Cast<ABHGamePlayerController>(PlayerOwner);
//	if (BHGamePC == NULL)
//	{
//		UE_LOG(LogBHGame, Warning, TEXT("Unable to create chat widget - Invalid player controller"));
//	}
//	else
//	{
...
//		{
//			bCreated = true;
//			FLocalPlayerContext WorldContext(BHGamePC);
//			GEngine->GameViewport->AddViewportWidgetContent(
...
//bool ABHGameHUD::IsMatchOver() const
//{
//	return GetMatchState() == EBHGameMatchState::Lost || GetMatchState() == EBHGameMatchState::Won;
//}

//void ABHGameHUD::AddMatchInfoString(const FCanvasTextItem InInfoItem)
//{
//	InfoItems.Add(InInfoItem);
//}

//float ABHGameHUD::ShowInfoItems(float YOffset, float TextScale)
//{
//	float Y = YOffset;
...
//float ABHGameHUD::DrawRecentlyKilledPlayer()
//{
