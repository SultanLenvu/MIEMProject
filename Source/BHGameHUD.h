...
struct FDeathMessage
{
	...
	TWeakObjectPtr<class UBHGameDamageType> DamageType;
...
public:
	...
	/** Sent from BHGameWeapon, shows NO AMMO text. */
	 ...
	 */
	//void SetMatchState(EBHGameMatchState::Type NewState);

	/** Get state of current match. */
	//EBHGameMatchState::Type GetMatchState() const;
	 ...
	 */
	void ShowDeathMessage(class ABHGamePlayerState* KillerPlayerState, class ABHGamePlayerState* VictimPlayerState, const UDamageType* KillerDamageType);
...
protected:
	...
	/** State of match. */
	EBHGameMatchState::Type MatchState;

	/** Is the scoreboard widget on screen? */
	uint32 bIsScoreBoardVisible : 1;

	/** Scoreboard widget. */
	TSharedPtr<class SBHGameScoreboardWidget>	ScoreboardWidget;
