...
#include "BHGame/Customization/Modules/ScopeBase.h"
#include "Camera/CameraComponent.h"
#include "Engine/StreamableManager.h"

#include "BHGameWeapon.generated.h"

class UAnimMontage;
class ABHGameCharacter;
...
class ABHGameWeapon : public AActor
{
	...
	virtual void OnEquip(const ABHGameWeapon* LastWeapon);
	...
	virtual void OnEnterInventory(ABHGameCharacter* NewOwner);
	...
	UFUNCTION(BlueprintCallable, Category = "Game|Weapon")
		class ABHGameCharacter* GetPawnOwner() const;
	...
	void SetOwningPawn(ABHGameCharacter* ABHGameCharacter);
...
protected:
	...
	UPROPERTY(EditDefaultsOnly, Category=Animation)
	UAnimMontage* ZatvorAnim;
private:
	/** weapon mesh: 1st person view */
	UPROPERTY(VisibleDefaultsOnly, Category = Mesh)
		USkeletalMeshComponent* Frame;

	void OnMeshLoaded(USkeletalMeshComponent* Component, TSharedPtr<FStreamableHandle>* MeshHandle);
	/** getting right mesh component for FX attach point**/
	USkeletalMeshComponent* GetMuzzelEdgeComponent();
	UPROPERTY(VisibleDefaultsOnly, ReplicatedUsing = OnRep_PathArray)
		TArray<FName> PathArray = TArray<FName>();
	UFUNCTION()
		void OnRep_PathArray(const TArray<FName>& PArray);
	void SetupModuleMeshes(const TArray<FName>& Paths);
	/** module components **/
	UPROPERTY(VisibleDefaultsOnly)
		USkeletalMeshComponent* MuzzelComp;
	UPROPERTY(VisibleDefaultsOnly)
		USkeletalMeshComponent* ForeGripComp;
	UPROPERTY(VisibleDefaultsOnly)
		USkeletalMeshComponent* StockComp;
	UPROPERTY(VisibleDefaultsOnly)
		USkeletalMeshComponent* HandleComp;
	UPROPERTY(VisibleDefaultsOnly)
		USkeletalMeshComponent* GripComp;
	UPROPERTY(VisibleDefaultsOnly)
		AScopeBase* ScopeComp;
	UPROPERTY(VisibleDefaultsOnly)
		ASkeletalMeshActor* MagazineComp;
	UPROPERTY(VisibleDefaultsOnly)
		TSubclassOf<UObject> MagazineClass;
	UPROPERTY(VisibleDefaultsOnly)
		USkeletalMeshComponent* GunPointComp;

	/** Handles for loading module assets**/
...
protected:
	...
	UFUNCTION(Server, Reliable, WithValidation)
		void SetupModulesServer(const TArray<FName>& Paths, const TArray<FName>& SocketNameArray);

	void ExtractModulesData();

	void SetupSkeletalMeshComponent(USkeletalMeshComponent*& Component);
	...
	virtual void FireWeapon() PURE_VIRTUAL(ABHGameWeapon::FireWeapon, );
...
protected:
	...
	FORCEINLINE USkeletalMeshComponent* GetMesh1P() const { return Frame; }
};
